...
def build_laplacian_pyramid(img, levels):
    ...
    for i in range(levels - 1):
        ...
        # Correcting the order of width and height in resize
        gaussian_resized = cv2.resize(gaussian_pyramid[i], (width, height))
        diff = cv2.subtract(gaussian_resized, upsampled)
...
def collapse_laplacian_video_pyramid(video, frame_ct):
    ...
    for i in range(frame_ct):
        ...
        for level in range(len(video) - 1, 0, -1):
            ...
            prev_level_frame_resized = cv2.resize(prev_level_frame, (width, height))  # Correct order of dimensions
            prev_frame = pyr_up_frame + prev_level_frame_resized
